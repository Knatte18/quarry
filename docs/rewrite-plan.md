...
produced them. `docs/glyph.md` is the identifier contract. Current operator state lives in the local,
gitignored `.scratch/handoff.md` (regenerated by `/mill:handoff`). `docs/roadmap.md` is what
happens next. This document is the distilled record of what quarry
