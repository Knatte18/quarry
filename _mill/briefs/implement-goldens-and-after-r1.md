# Implementer Brief — Facade + CLI, toc (T5a) / goldens-and-after
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/quarry/wts/facade-cli-toc/_mill/plan/05-goldens-and-after.md`
- **Overview (for `## Shared Decisions` only):** `/home/knatte/Code/quarry/wts/facade-cli-toc/_mill/plan/00-overview.md`
- **Worktree cwd:** `/home/knatte/Code/quarry/wts/facade-cli-toc`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/quarry/wts/facade-cli-toc log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/knatte/Code/quarry/wts/facade-cli-toc log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/quarry/wts/facade-cli-toc/_mill/plan/05-goldens-and-after.md`.
   - Commit the plan edit first (`plan: extend goldens-and-after refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"90342950-b8bb-4afc-9a9e-1acce809e46a"}
   ```
...
**Pre-report self-check (mandatory before emitting success JSON):** Run `git -C /home/knatte/Code/quarry/wts/facade-cli-toc status --porcelain --untracked-files=no`.
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/quarry/wts/facade-cli-toc log --grep="^mill-go: start batch" -n 1 --format=%H`.
Run `git -C /home/knatte/Code/quarry/wts/facade-cli-toc log <range-start>..HEAD --oneline` and match commit subjects against the batch file's `## Cards` `Commit:` messages to get an exact count -- Commit: none cards are never expected to appear in this log;
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"90342950-b8bb-4afc-9a9e-1acce809e46a","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `90342950-b8bb-4afc-9a9e-1acce809e46a` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"90342950-b8bb-4afc-9a9e-1acce809e46a","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"90342950-b8bb-4afc-9a9e-1acce809e46a"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/quarry/wts/facade-cli-toc` for commits;
do not `cd`.

## Path format

**File paths are POSIX-style relative paths from `/home/knatte/Code/quarry/wts/facade-cli-toc`.**
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/quarry/wts/facade-cli-toc show <parent-branch>:<path>` to read it without changing cwd.
