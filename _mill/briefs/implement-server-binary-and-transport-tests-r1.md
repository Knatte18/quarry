# Implementer Brief — Add an MCP wrapper for quarry / server-binary-and-transport-tests
...
- **Batch file (authoritative for this batch):** `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper/_mill/plan/06-server-binary-and-transport-tests.md`
...
Before implementing any cards, identify which cards are already committed: run `git -C /home/knatte/Code/quarry/wts/quarry-mcp-wrapper log ..HEAD --oneline` and match each commit subject against the cards' `Commit:` messages.
When `` is empty, derive the range start via `git -C /home/knatte/Code/quarry/wts/quarry-mcp-wrapper log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
2. If you discover that a card must touch a file not listed in any of its `Context:`/`Edits:`/`Creates:` lists:
   ...
   - Add the file to the appropriate list in `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper/_mill/plan/06-server-binary-and-transport-tests.md`.
   - Commit the plan edit first (`plan: extend server-binary-and-transport-tests refs for <short reason>`) and push via the wiki.
...
4. If you are forced to stop before all cards are committed (e.g. approaching context limit or an unresolvable error), emit the following JSON as your very last output line and then stop — do not report `success`:
   ...
   ```json
   {"status":"incomplete","cards_completed_count":N,"cards_remaining":M,"session_id":"13107a2d-1d5c-4256-ac33-a94051ab5121"}
   ```
...
Determine the range start exactly as in "Resume-after-incomplete" above: use `` when non-empty, else `git -C /home/knatte/Code/quarry/wts/quarry-mcp-wrapper log --grep="^mill-go: start batch" -n 1 --format=%H`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"13107a2d-1d5c-4256-ac33-a94051ab5121","cards_done":[<card numbers this commit set addresses>]}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `13107a2d-1d5c-4256-ac33-a94051ab5121` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"success","commit_sha":"<HEAD-sha, unchanged from before this turn>","session_id":"13107a2d-1d5c-4256-ac33-a94051ab5121","cards_done":[<every card number declared in this batch>],"already_complete":true}
```

or, when stuck:

```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"13107a2d-1d5c-4256-ac33-a94051ab5121"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
