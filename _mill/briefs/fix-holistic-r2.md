...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/ladder-breadth/_mill/reviews/20260904-185127-code-review-r2.md`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/ladder-breadth/_mill/reviews/20260904-185127-code-review-r2.md`.
...
- `--cells` run example re-includes the retired ladder-a cells: **Location:** `bench/loomyard-eval/ladder/ladder-toc.yaml:75` (the "Run with:" block's `--cells` line)
...
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/quarry/wts/ladder-breadth status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"ab8303e6-4f20-4f82-aaef-ae7523977df8"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `ab8303e6-4f20-4f82-aaef-ae7523977df8` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"ab8303e6-4f20-4f82-aaef-ae7523977df8"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
