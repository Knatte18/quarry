# Holistic Fixer Brief — Thin quarry/ facade over internal/quarryengine
...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/quarry-thin-facade/_mill/reviews/20260827-155431-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/quarry/wts/quarry-thin-facade/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/quarry/wts/quarry-thin-facade`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/quarry-thin-facade/_mill/reviews/20260827-155431-code-review-r2.md`.
...
- Close's doc comment is attached to the wrong method: **Location:** `internal/quarryengine/lsp/lspclient.go:584-597`
...
Run each from `/home/knatte/Code/quarry/wts/quarry-thin-facade` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/quarry/wts/quarry-thin-facade rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/quarry/wts/quarry-thin-facade status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"0d08d3b3-cd8b-4706-bb7b-f50d42e07fa7"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `0d08d3b3-cd8b-4706-bb7b-f50d42e07fa7` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"0d08d3b3-cd8b-4706-bb7b-f50d42e07fa7"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
