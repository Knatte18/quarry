# Holistic Fixer Brief — Per-capability quarry-mcp benchmark suite
...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/mcp-capability-bench/_mill/reviews/20260829-152840-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/quarry/wts/mcp-capability-bench/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/quarry/wts/mcp-capability-bench`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/mcp-capability-bench/_mill/reviews/20260829-152840-code-review-r2.md`.
...
- cold-cell disposition strings disagree between batch 5 and batch 6: **Location:** `bench/loomyard-eval/ladder/scripts/run_ladder.py:1008,1011,1017,1023` vs `bench/loomyard-eval/ladder/scripts/summarize.py:294,414`
- gates.py reimplements tool_use/tool_result traversal instead of reusing extract_usage.iter_tool_uses: **Location:** `bench/loomyard-eval/ladder/scripts/gates.py:77-98` (`_tool_use_blocks`, `_tool_results_by_id`) vs `bench/loomyard-eval/ladder/scripts/extract_usage.py:60-70` (`iter_tool_uses`)
- run_probe's denial check duplicates gate_denied_tools_not_used's predicate: **Location:** `bench/loomyard-eval/ladder/scripts/run_ladder.py:398-419` (`_denied_call_succeeded`) vs `bench/loomyard-eval/ladder/scripts/gates.py:101-124` (`gate_denied_tools_not_used`)
...
Run each from `/home/knatte/Code/quarry/wts/mcp-capability-bench` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/quarry/wts/mcp-capability-bench rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/quarry/wts/mcp-capability-bench status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"e63ad153-daf9-43cc-b52a-9c0c8cdccefd"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `e63ad153-daf9-43cc-b52a-9c0c8cdccefd` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"e63ad153-daf9-43cc-b52a-9c0c8cdccefd"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
...
Use `git -C /home/knatte/Code/quarry/wts/mcp-capability-bench` for commits;
do not `cd`.
...
- **If you need a file from the parent:** use `git -C /home/knatte/Code/quarry/wts/mcp-capability-bench show <parent-branch>:<path>` to read it without changing cwd.
