# Holistic Fixer Brief — Ladder, toc rerun (T7)
...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/ladder-toc-rerun/_mill/reviews/20260904-151805-code-review-r2.md`
- **Plan overview:** `/home/knatte/Code/quarry/wts/ladder-toc-rerun/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/quarry/wts/ladder-toc-rerun`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/ladder-toc-rerun/_mill/reviews/20260904-151805-code-review-r2.md`.
...
- Matrix's one invocation violated the unconditional clean-tree rule, and the root was not voided: **Location:** `bench/loomyard-eval/ladder/results/2026-09-04-toc/provenance.json:5-8` (`quarry_dirty: true`, `quarry_dirty_files: ["_mill/briefs/implement-matrix-run-r1.md"]`); `bench/loomyard-eval/ladder/results/2026-09-04-toc/conclusion.md:157-169`; `.scratch/ladder-toc-run.log:41-42`
...
Run each from `/home/knatte/Code/quarry/wts/ladder-toc-rerun` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/quarry/wts/ladder-toc-rerun rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/quarry/wts/ladder-toc-rerun status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"42132855-b216-48da-940c-0374a6c5997c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `42132855-b216-48da-940c-0374a6c5997c` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"42132855-b216-48da-940c-0374a6c5997c"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
