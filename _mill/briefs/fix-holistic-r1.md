...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/mcp-capability-bench/_mill/reviews/20260829-151321-code-review-r1.md`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/mcp-capability-bench/_mill/reviews/20260829-151321-code-review-r1.md`.
...
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/quarry/wts/mcp-capability-bench status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"6c575eeb-587d-4f3f-a096-aee4d4f2f97b"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `6c575eeb-587d-4f3f-a096-aee4d4f2f97b` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"6c575eeb-587d-4f3f-a096-aee4d4f2f97b"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
