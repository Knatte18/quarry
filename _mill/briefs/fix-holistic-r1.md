# Holistic Fixer Brief — Add an MCP wrapper for quarry
...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper/_mill/reviews/20260829-103018-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper/_mill/reviews/20260829-103018-code-review-r1.md`.
...
Run each from `/home/knatte/Code/quarry/wts/quarry-mcp-wrapper` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/quarry/wts/quarry-mcp-wrapper rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made), unless every finding was a legitimate --nits-only no-op requiring no code change. Run `git -C /home/knatte/Code/quarry/wts/quarry-mcp-wrapper status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"4163ed40-37d8-4580-b6cc-0b27caac1bb9"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `4163ed40-37d8-4580-b6cc-0b27caac1bb9` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"4163ed40-37d8-4580-b6cc-0b27caac1bb9"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
