...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/quarry-thin-facade/_mill/reviews/20260827-132848-code-review-r1.md`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/quarry-thin-facade/_mill/reviews/20260827-132848-code-review-r1.md`.
...
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/quarry/wts/quarry-thin-facade status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"463199dc-d40e-4c7f-9d18-12acd41d7f11"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `463199dc-d40e-4c7f-9d18-12acd41d7f11` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"463199dc-d40e-4c7f-9d18-12acd41d7f11"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
