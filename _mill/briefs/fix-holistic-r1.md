# Holistic Fixer Brief — Ladder harness around headless claude -p (T2)
...
- **Holistic review file:** `/home/knatte/Code/quarry/wts/ladder-harness/_mill/reviews/20260903-194737-code-review-r1.md`
- **Plan overview:** `/home/knatte/Code/quarry/wts/ladder-harness/_mill/plan/00-overview.md`
- **Worktree cwd (use for git and verify):** `/home/knatte/Code/quarry/wts/ladder-harness`
...
Load the **mill-receiving-review** skill before reading any finding in `/home/knatte/Code/quarry/wts/ladder-harness/_mill/reviews/20260903-194737-code-review-r1.md`.
...
Run each from `/home/knatte/Code/quarry/wts/ladder-harness` via Bash.
...
**Pre-report self-check (mandatory before emitting success JSON):** At the very start of your session (before any edit), record `git -C /home/knatte/Code/quarry/wts/ladder-harness rev-parse HEAD` as your baseline -- this is the holistic fix housekeeping commit (message starts with `mill-go: holistic fix`).
Before reporting `success`, confirm HEAD now differs from that recorded baseline;
never report `success` when HEAD equals the baseline (no new commit was made). Run `git -C /home/knatte/Code/quarry/wts/ladder-harness status --porcelain --untracked-files=no`.
...
```json
{"status":"success","commit_sha":"<last-HEAD-sha>","session_id":"8d9fb2fb-4532-4a87-874c-d36e3f5ccbbe"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**

**`session_id` MUST be exactly `8d9fb2fb-4532-4a87-874c-d36e3f5ccbbe` (the UUID shown in the example above — it was injected into this brief when it was rendered).
Copy it verbatim.**
...
```json
{"status":"stuck","stuck_type":"transient|verify|logic","reason":"<one-line>","commit_sha":"<last-HEAD-sha>","session_id":"8d9fb2fb-4532-4a87-874c-d36e3f5ccbbe"}
```
**Do not wrap the JSON in a code block.
Output it as a bare line — no backticks, no fence.
Anything other than a bare JSON line is treated as `stuck_type: logic`.**
