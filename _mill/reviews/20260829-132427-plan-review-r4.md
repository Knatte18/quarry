...
### [NIT:consistency] `resolve_state_dir` halts on env the harness already scrubs
...
### [NIT:decision] `max_turns` and the scorer pin carry no values and no gate
**Demoted-from:** BLOCKING
...
### [NIT:decision] Fasit free-text fields name quarry; only `_meta` is stripped
**Demoted-from:** BLOCKING
...
_Note: 3 finding(s) demoted from BLOCKING to NIT by the stage's blocking-class ceiling; current blocking_count is 4._
MILL_REVIEW_END
