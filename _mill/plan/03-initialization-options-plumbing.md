...
- **Requirements:**
  ...
  - Update `internal/quarryengine/daemon/doc.go` in three places, not one. The line spelling out `EnsureServer`'s signature must match the new parameter list. The native-strategy paragraph, which describes `ensureNative` as spawning a disposable local proxy that gopls itself dedups behind a real shared daemon, and the `ConnKindNative` bullet, which says closing the connection ends only this session and never gopls's shared daemon, are both falsified for a non-empty tag set — a private spawn has no shared daemon behind it. Qualify both to say they describe the empty-tag-set case, and state what a non-empty tag set does instead.
