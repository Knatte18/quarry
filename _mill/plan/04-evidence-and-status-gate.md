...
- **Requirements:**
  ...
  Run the three steps in exactly this order. The order is the whole point, because the regeneration flag rewrites **every** row's file, the four already-committed table-of-contents files included — so regenerating first and comparing afterwards would compare regenerated bytes against themselves and prove nothing:

  1. **First**, run the same command **without** the regeneration flag. Each row is its own subtest, so the four table-of-contents rows compare against their committed bytes and must pass, while the eight new rows fail on a missing golden. Four passing table-of-contents rows at this point is this task's proof that the shared-encoder and symbol-line refactors changed nothing — it is the only moment that proof can be taken, and it must be taken before anything is rewritten.
  2. **Then** run it with the regeneration flag to produce the eight new files.
  3. **Then** confirm the four table-of-contents goldens were not altered by that run, with `git diff --exit-code` scoped to those four paths. A non-empty diff there means a refactor did change the bytes and the step-1 result was misread; stop and report rather than committing the rewrite.

  Finally, re-run without the regeneration flag and confirm all twelve rows pass.
...
Card 16 is the one card whose work is not proved by the batch verify alone: it is verified by its own ordered three-step procedure on the machine that produced the files — the unregenerated comparison first, which is where the four table-of-contents goldens prove the refactors changed nothing, then the regeneration, then a scoped `git diff --exit-code` confirming those four files were not rewritten by it.
