...
- **Requirements:**
  ...
  - Verification is skipped entirely — every reference kept, `declaration` returned as whatever the definition call produced — in each of these cases, and this list is exhaustive: `opts.SkipVerification` is set; the declaration-side definition call errored; the declaration-side definition call returned an empty location set; `client.SupportsImplementation()` reports false; the implementation call errored; and, in directional mode only, `client.SupportsDocumentSymbol()` reports false, a `documentSymbol` call errored, or a `documentSymbol` call returned an empty result. In every error case, set the timed-out flag if the error is a server timeout, then continue rather than returning the error.
