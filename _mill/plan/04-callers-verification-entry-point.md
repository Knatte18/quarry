...
- **Requirements:**
  ...
  - Write `internal/quarryengine/query/verify_test.go` covering, as tables over hand-built `lsp.Location` values: a reference whose definition matches the match set is kept; one whose definition points elsewhere is dropped; one whose definition call errored is kept; one whose definition returned an empty list is kept; one never attempted is kept; two same-named declarations one line apart in the same file are distinguished, proving the match is positional and not file-level; and an empty match set drops every attempted reference whose definition returned a non-empty, non-matching result — assert that directly, because it is exactly why card 15 must never call this function with an empty match set and skips verification instead.
...
- **Requirements:**
  ...
  - Widen `internal/quarryengine/query/refs.go`'s package doc comment in the same edit. It currently says the package implements "References, Definition, and Symbol, the engine's public orchestration entry points", which misses `Callers`, and says `lookup` is wrapped by callers that "differ only in which single LSP call they make", which card 13's `runOnConnection` and this card's multi-call pipeline both retract. Name `Callers` and `runOnConnection` and drop the single-call claim. This is the same defect card 1 forbids for the `lsp` package doc, in the file this card edits.
  - When `opts.SkipVerification` is set, elide the phases that exist only to serve verification: issue `textDocument/definition` and `textDocument/references` and nothing else. The definition call still runs because its result is the returned `declaration` value the CLI's declaration exclusion depends on; `textDocument/implementation` — and, in directional mode, the whole `documentSymbol` classification phase — feed only the match set, which is unused when nothing is filtered. State this explicitly in the doc comment: `--no-verify` is then exactly today's two-call behaviour on one connection instead of two.
  ...
  - Verification is skipped entirely — every reference kept, `declaration` returned as whatever the definition call produced — in each of these cases, and this list is exhaustive: `opts.SkipVerification` is set; the declaration-side definition call errored; the declaration-side definition call returned an empty location set; `client.SupportsImplementation()` reports false; the implementation call errored; and, in directional mode only, `client.SupportsDocumentSymbol()` reports false, a `documentSymbol` call errored, a `documentSymbol` call returned an empty result, or the `documentSymbol` classification phase hit its own deadline. In every error case, set the timed-out flag if the error is a server timeout, then continue rather than returning the error.
  - Those three directional-mode cases are skip triggers for the same reason the implementation half's are: each one makes `isInterfaceDeclaration` false for every implementation location, which narrows the match set and therefore *drops* references that a healthy server would have kept — a degraded server making the gate greener, which is precisely what the `verification-is-fail-closed-everywhere` Shared Decision forbids. Never narrow the match set on a degraded classification; skip instead.
  - When verification does run, build the match set with `declarationMatchSet` as card 14 declared it. If card 14 implemented the directional shape, first gate on `client.SupportsDocumentSymbol()`, then issue one `textDocument/documentSymbol` per distinct file URI among the implementation results and classify each implementation location with `isInterfaceDeclaration`. Bracket that whole classification phase in its own `context.WithTimeout(ctx, timeout)`, one deadline for the phase rather than one per file, exactly as the per-reference verification loop is bracketed — every LSP phase in this pipeline is deadline-bounded, and a phase that could block indefinitely would contradict both the package doc's own claim and `lookup`'s. A deadline hit here is one of the skip triggers above: keep every reference, and set the timed-out flag as a server timeout on that phase would.
  - The capability gate mirrors `resolvePosition` in `internal/quarryengine/query/refs.go`, which already gates its own `documentSymbol` use on exactly that accessor — but the disposition differs deliberately: `resolvePosition` cannot resolve at all without it and raises `ErrResolverUnsupported`, whereas here a missing capability has a strictly safer answer available, so it skips verification rather than failing the call.
...
- **Requirements:**
  ...
  - Update that file's package doc comment in the same edit. It says the only callers outside package `daemon` are `query`'s `refs_test.go` and `refs_integration_test.go`; this card adds `callers_test.go` and batch 6's card 24 adds `buildtags_lsp_test.go`. Name all four, or reword the sentence so it stops enumerating callers it cannot keep current.
...
- **Requirements:**
  ...
  - `quarry/facade_test.go` carries three more checkable counts, and this card's own additions falsify all three: the `init` comment saying "each of the fourteen aliased types" becomes fifteen, the comment above the blank-identifier block saying "The eight blank-identifier assignments below" becomes ten, and `TestFacadeSentinels_Identity`'s doc comment saying "each of the seven re-exported sentinel error values" becomes eight. Update all three in the same edit, on the same rationale: a checkable count left stale is worse than no count.
