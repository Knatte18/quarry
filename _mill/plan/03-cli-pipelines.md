...
- **Requirements:**
  ...
  In `internal/cli/cli_test.go`, extend the fixture the pipeline tests share so it also carries a package declaring one free function and one type with a method, so the resolve verb has something spellable to answer about. Put that package in a **new sibling directory** of the fixture's existing nested one, not inside it: an existing flag-pass-through case asserts that the existing nested directory holds exactly one file entry, so adding a file there would break an assertion this same card requires to keep passing. Do not put it at the fixture root either — a file directly under the root has the empty unit, which no glyph can spell. Add no build-tag-duplicated declaration to this fixture: two declarations of one name resolve to the ambiguous status, which would contradict this card's own first case, and the ambiguous status is covered by the self-built tree in batch 4 instead. Add end-to-end cases running the resolve verb with explicit root flags and buffer sinks, asserting exit code, stdout bytes and stderr bytes together:
  ...
  - a path escaping the root: exit 1, a payload carrying the engine's own error string asserted as an exact byte-for-byte expected value, not as a substring match, and empty stderr. This is the only test anywhere in the plan that pins that string, so it is what stops an implementer from trimming the doubled prefix the overview's payload-error-text decision forbids touching;
  ...
  - the target-echo asymmetry, covered in one test so it is pinned rather than discovered: an **absolute** path target under the root, asserting the payload's target field echoes the repository-relative form rather than the argument, together with a glyph target asserting its target field echoes the argument verbatim. Use an absolute target, not one relative to a subdirectory: every test here passes an explicit root flag, which makes the base directory the root itself, so a relative target's repository-relative form is the argument and the assertion would hold vacuously. Changing the process working directory to make it non-vacuous is forbidden, so the absolute form is the reachable one.
