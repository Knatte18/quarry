...
- **Requirements:** Add `run_probe(ladder, repo_root, results_root, target_dir, server_path)`, executed once before any matrix run. It launches one throwaway `claude -p` run with the quarry server declared, a deny-list naming `mcp__quarry__impact`, and a prompt instructing the agent to call that denied tool and report what happened. It writes `probe.json` into the results root recording:
  ...
  - `denied_tools_advertised` — whether the denied name appears in the init event's `tools` array. When it does not, denied tools are hidden from the model and `denied_tool_attempts` can never be non-zero. The metric is still summarised either way — `summarize.py` reads this boolean as `denied_tool_attempts_reported` and reports the column beside it — because a flag that qualified a metric the summary omitted would qualify nothing. What the flag decides is whether a column of zeros may be read as evidence: when denied tools are hidden it may not, and the conclusion says so rather than presenting the zeros as a finding.
  - `advertised_tools`, `session_id`, the resolved model, the resolved `QUARRY_CONFIG` value, and the probe's own transcript path, so the answer is auditable rather than asserted.

  The probe's transcript is captured to `<results_root>/raw/probe/transcript.jsonl`, and `probe.json` records that path. It goes under `raw/` deliberately: that is the one subtree the `.gitignore` pattern covers, so the transcript stays untracked like every other per-run transcript while `probe.json` beside it at the results root stays tracked. A transcript written directly at the results root would be tracked, undeclared by any card's `Creates:`, and would leave the probe card's commit either dirty or committing a file the plan never named.
...
- **Requirements:** Add:
  ...
  - `execute_run(ladder, config, n, run_dir, target_dir, server_path, repo_root, cache_dir, executor=launch_run)` — the per-run pipeline, in this order: materialise inputs; record a monotonic start time; launch the subprocess through `executor`, capturing its stdout stream directly to `transcript.jsonl` as it runs; record wall clock; parse the final fenced json block out of the result event's `result` field into `answer.json`; extract `usage.json`; run `run_gates`, passing the `ladder` it already holds and the scrubbed `run_env()` mapping, which is where `observe_worktree_dirtied` is taken — before any restore, since the restore is what erases the evidence; invoke `score_run`; run `gate_run_complete_artifacts`; and only then `write_run_json`, carrying the gate report's non-fatal observations into it. `run_gates` runs before scoring and therefore asserts on no scoring artifact; `gate_run_complete_artifacts` is the separate later gate that requires all four files. A run whose result event reports a non-`success` `subtype` — the shape a run stopped by `--max-turns` leaves, with no fenced json block to parse — returns a distinct `truncated` outcome rather than the generic unparseable-answer failure, because the two call for opposite responses. Any failing fatal gate, an unparseable answer from a run that did finish, or a scoring failure returns a failed outcome carrying the findings, and no `run.json` is written in any of these cases.
...
- **Requirements:** Add:
  ...
  - `MAX_ATTEMPTS = 3` and `run_matrix(ladder, results_root, worktrees, server_path, repo_root, cache_dir, executor=launch_run, git=run_git)` — iterate `pending_runs(main_runs(ladder), results_root)` one at a time, never concurrently. For each: call `warm_daemon` against that run's task worktree, then execute; on success continue; on failure `invalidate` the run directory and retry, up to `MAX_ATTEMPTS` attempts for that run. On the third failure, halt the **whole** matrix — not just that cell — reporting which gate failed and why, and leaving every completed run intact so the matrix resumes once the cause is fixed.

    A `truncated` outcome is **not** retried and halts the matrix on its first occurrence, naming the config, the repetition, and the `max_turns` value. Retrying is deterministic waste — the ceiling is a matrix-wide constant, so a second and third attempt hit it identically and burn two more paid runs before halting anyway — and, more importantly, a truncation means the constant is wrong for this matrix: every already-completed run was measured under a ceiling one of its peers could not meet, so the comparison they were for is void. A config that legitimately needs more than the ceiling (plausibly a `none` control on task 01's multi-package survey) is a real finding about that rung, and the operator's remedy is to raise `max_turns` and re-run the matrix into a **new dated results directory**, since a matrix spanning two ceilings is not a matrix. Halting on the first occurrence rather than the third is the same reasoning the three-attempt cap rests on, applied to a failure already known to be deterministic. After every main-matrix run, successful or not, call `restore_worktree` unconditionally. The `worktree_dirtied` observation is **not** taken here — `execute_run` already recorded it before the restore, and an observation taken after a `reset --hard` plus `clean -fdx` would be `False` for every run in the matrix. Dirtying is never itself a failure.
...
- **Requirements:** Add `run_cold_cell(ladder, results_root, server_path, repo_root, cache_dir, executor=launch_run, git=run_git)`, executed after the entire main matrix. For each of the cold config's repetitions:
  - Before the first repetition, drain the daemons the main matrix left resident: call `wait_for_daemon_exit` on each shared task worktree's state directory with the same `DAEMON_EXIT_TIMEOUT_S` bound used between repetitions. The argument for waiting between cold runs applies one boundary earlier and with more force — the main matrix has just finished, so both task daemons are certainly resident, and they would be competing for CPU with exactly the runs the warm-versus-cold contrast reads.
  ...
  - Run **no** warm-up step.
  - Before launching — and before each retry attempt, not just the first — call `clear_state_dir` on the run's resolved state directory, then assert `gate_cold_before`. The clear is what makes a retry possible at all: a failed attempt that started a supervised daemon leaves its `daemon.json` behind, since neither the state file nor its directory is removed on exit, so without the clear attempts 2 and 3 would fail the precondition deterministically and land in none of the driver's dispositions. `gate_cold_before` itself keys on pid liveness rather than file presence, so a stale leftover is not mistaken for a warm daemon. A genuinely live daemon means the run cannot be cold, and it is invalidated rather than reported as cold.
  ...
  Four dispositions, written to that record and also returned to the caller. Each names how many of the `reps` repetitions were confirmed-cold, so a cell that is neither uniformly good nor uniformly bad has somewhere to land instead of falling between the categories. Confirmed-cold: at least one repetition was a supervised cold run. Not-run: the supervised strategy is unavailable on this machine, every attempt having failed `gate_cold_after` on the native-fallback branch — an environment limitation, not a fault, so the cell is reported as not run rather than with numbers that cannot be trusted, and the matrix is not halted. No-daemon-signal: all three repetitions completed validly but none invoked a daemon-backed tool, so the cell holds three good runs that say nothing about warmth; this is distinct from not-run precisely because the runs are valid and are still summarised — what is absent is the contrast, not the data. Partial: between one and `reps - 1` repetitions were confirmed cold and the rest exhausted `MAX_ATTEMPTS` on the native-fallback branch, which is the mixed outcome a machine that takes the native path intermittently produces. A partial cell is recorded with its confirmed count, is **not** listed as incomplete — it is a legitimate terminal state, not an interrupted run — and yields no warm-versus-cold comparison, because a disjoint-range claim at `n = 3` cannot be made from fewer than three cold runs.
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Add the CLI under `if __name__ == "__main__":` taking a ladder path, a results root, and a `--stage` argument accepting exactly `probe`, `main`, `cold`, or `all`, defaulting to `all`. Every stage first calls `require_pins`, builds the server, and runs `ensure_task_worktrees`; then `probe` runs the probe and stops, `main` runs the main matrix, `cold` runs the cold cell, and `all` runs the three in that order before printing where `summarize.py` should be pointed. The selector is load-bearing rather than a convenience: batch 8 executes the probe, the main matrix, and the cold cell as three separate cards with their own commits, and without it the only way to reach a card boundary would be to interrupt a running matrix, which is not a boundary an operator can aim at. Any stage that includes the probe skips it when a `probe.json` already present in the results root records a passing probe — the CLI is re-invoked on every resume, and without the skip each resume would spend a paid run re-probing and overwrite an already-committed probe record. The CLI refuses to start when any pinned value is unset, halting with a message naming the field and the ladder file.
