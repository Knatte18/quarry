...
- **Requirements:** add `IsDirectiveBlock(lang string, startLine int, stripped string) bool`,
  ...
  - `python`: a `#!` shebang, or a PEP 263 coding line matching `coding[:=]`. Both are directives only
    on their original first-or-second physical line, which is what `startLine` (the block's 1-based
    starting line) is for — a `#!`-shaped line in the middle of a file is prose, not a shebang.
...
- **Requirements:** table-driven tests, written before the implementations they cover are considered
  ...
  shebang at line 1; a `#!`-shaped line starting at line 12, asserting it is not treated as a shebang;
