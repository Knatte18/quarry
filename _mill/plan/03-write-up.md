...
- **Requirements:** A zero-diff verification card that repeats, once, the spot check T2's own
  ...
  numbered one that is, adjusting the paths above accordingly. From its transcript, count the turns,
...
- **Requirements:** Write the conclusion in the shape of the `v1-final` toc conclusions: a header
  ...
  **Cover every one of these regardless of outcome:** any cell short of 5 repetitions and why; every
  ...
  any session-fingerprint drift; the number of invocations the matrix took and whether any repetition
  was attempt-exhausted; the result of card 5's hand verification; and whether `output_tokens` is
  usable on this host, since it was unusable on the V1 host.
  ...
  **Record plan §11's decision:** the raw tree stays untracked, confirming the `results/*/raw/`
  ...
  If the matrix restarted into a `-r2` root, name the harness fix and the abandoned root, and check
  that the abandoned directory carries its `ABANDONED.md`. If the run stopped on a defect in the code
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** In §10's successor section, "Open decisions", replace the third bullet — the one
  reading "Whether the new harness's `results/**/raw/` is committed or ignored; T2 decides when it
  writes its first root" — with the decision and its reason: the raw tree is ignored, settled by this
...
- **Deletes:** none
- **Moves:** none
- **Requirements:** Three edits, no more.
  ...
  points at records, it does not duplicate them. Change no other section.
...
summary, the provenance record or the rendered table committed beside it. The repository-wide done
gate (`go test ./... && golangci-lint run`, with the live-test guard unset) runs after this batch and
covers the tree as a whole.
