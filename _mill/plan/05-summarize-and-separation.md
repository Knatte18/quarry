...
- **Requirements:** Write the tests first. Add `build_summary(ladder, results_root)` returning the full mapping written to `summary.json`:
  ...
  A cold cell whose `cold_cell.json` records `not-run` is **excluded from the `incomplete` list**: it has zero complete runs by design, and without that record it is indistinguishable from a cell the operator interrupted. Any other cell with fewer than `reps` complete runs is incomplete as before.
