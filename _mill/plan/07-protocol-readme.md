...
- **Requirements:** Write the protocol so a fresh agent with no memory of this design can execute and re-run the matrix from this file alone, mirroring the framing the sibling benchmark README opens with. Required sections:
  ...
  - **Run environment** — the pinned model, the identical allow-set, `--setting-sources ""`, `--strict-mcp-config`, the non-interactive permission mode, `QUARRY_STATE_DIR` and `QUARRY_BUILD_TAGS` both cleared, `QUARRY_CONFIG` deliberately left in place with the reason, `--state-dir` never passed, the per-run turn ceiling, and the per-run daemon warm-up for the main matrix. State why the two cleared variables matter: each would move the resolved state directory off the per-path key the cold cell's whole argument rests on.
