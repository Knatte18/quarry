...
- **Moves:** none
- **Requirements:**
  - Add `func resolveBuildTags(flagValue string) []string` to `internal/cli/paths.go`, resolving in the precedence `--build-tags` value, then `$QUARRY_BUILD_TAGS`, then empty, and returning `quarry.NormalizeBuildTags` applied to whichever won. That call needs a new `github.com/Knatte18/quarry/quarry` import in `internal/cli/paths.go`, which currently imports only the standard library; add it. The batch-local decision siting the function here rests on the flag-then-environment resolution shape it shares with `resolveConfigPath` and `resolveStateDir`, and on `os` already being imported — not on the file staying stdlib-only, which it does not. A raw value that normalizes away — `""`, `","`, `" , "` — yields nil, which every downstream consumer treats as "no build tags".
...
- **Requirements:**
  ...
  - Keep the filter composition after the call exactly as it is — `--within` via `filterWithin`, then `filterUnexpectedCallers` applying `--except` and the declaration exclusion in its single existing pass. Verification now happens inside `quarry.Callers`, ahead of all three, so the documented "`--within` before `--except`" relationship is preserved and no restructuring is implied. Leave the bodies of `filterWithin`, `isWithinDir` and `filterUnexpectedCallers` untouched.
  - `filterWithin`'s own doc comment is the one exception to that: it describes the function as "mitigating gopls' interface-method reference conflation across packages", which is the rationale card 21 retracts from the help text. Rewrite that doc comment to describe what the function does — restrict results to files under a directory — without the conflation rationale. Doc comment only; the body stays byte-identical.
