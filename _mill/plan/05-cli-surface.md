...
- **Requirements:**
  ...
  - Add a `buildTags []string` parameter to `buildOptions` in `internal/cli/cli.go` and set `Options.BuildTags` from it. Update all six existing `buildOptions` call sites to pass the resolved value.
