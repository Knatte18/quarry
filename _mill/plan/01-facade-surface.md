...
- **Requirements:**
  ...
  In `quarry/repo_test.go`, add tests beside the existing `TOC` tests, built with the same `writeScratchTree` helper those tests already use — but with one difference from their fixtures that this card must not get wrong: every Go file these new cases query must live in a **nested package directory**, not directly at the fixture root. The two sentinel fixtures in this file put a Go file directly at the fixture root, and a file directly under the root has the empty unit, which no glyph can spell; a glyph case built that way could never resolve. The two `TOC` answer fixtures already nest theirs under a subdirectory, which is the pattern to copy. Name the nested directory in the fixture and spell every glyph against it.
...
- **Requirements:**
  ...
  1. `r.Status == ""` — a pre-resolution rejection carried by the error field. One line and nothing else: the target as given, then a space, then the word `error`, then a space and the reason word only when the reason field is non-empty, then the normalised error string preceded by a colon and a space — but that colon, space and message are emitted only when the normalised error string is non-empty. With an empty reason this degenerates to `<target> error: <message>`; with an empty error field it degenerates to `<target> error` with no trailing colon and no trailing space. The empty-error form is unreachable for a value the engine produces, since it only ever sets this status-less shape alongside an error string, and it is spelled because emitting the colon unconditionally would leave a trailing space and break the no-trailing-whitespace invariant this renderer promises for every input. The identifier field is empty on this branch, which is why the line names the target rather than the identifier.
  2. `r.ID != ""` — a glyph target. On a not-found status, line 1 is the identifier, a space, the word `not_found`, and then — only when the unit field is non-empty — a space and the parenthesised clause `(unit found)` or `(unit not_found)` taken from it; nothing follows either way. The engine always sets the unit field on a glyph not-found, so the clause is always present in practice; the guard exists so an empty unit field emits no dangling space, under the same invariant branch 1's empty-error guard serves. On any other status, line 1 is the identifier, a space, and the status word; then one symbol line per entry, in order — the symbols slice for found and multipart, the candidates slice for ambiguous.
  ...
  2. found — line 1 is the identifier, a space, `found`; then the head's symbol line; then, only when the members slice is non-empty, one blank line followed by one symbol line per member in order. The blank line is the same block separator `dirBlocks` already uses, so no new marker is invented and the head stays distinguishable from the members without a key.
     ...
     The head is a pointer field, so guard it exactly as branch 3 of the resolve renderer guards its directory answer, and for the identical reason: when it is nil, emit line 1 alone and nothing else — no head line and no members. That is unreachable for a value the engine produces, and it is spelled because this is an exported renderer an external caller can hand a found answer carrying no head.
  ...
  In `quarry/text_test.go`, add a table per renderer covering every branch listed above as an exact expected string, and assert the no-trailing-whitespace-and-one-newline property with the file's existing `assertNoTrailingWhitespaceAndOneNewline` helper. Include all five totality guards in that table, each driven by a value an external caller could construct: the expand renderer's fall-through branch from a zero answer value; its found branch's nil-head guard from a found answer whose head pointer is nil; the resolve renderer's nil-directory-answer guard from a found path result whose pointer is nil; branch 1's empty-error guard from a status-less result whose error field is empty; and branch 2's empty-unit guard from a glyph not-found whose unit field is empty. Assert both invariants on every one of them — no trailing whitespace on any line, and exactly one trailing newline — since the last two guards exist specifically to keep the first invariant true. Include a path-target found case whose directory answer holds exactly one file entry — the shape the engine produces for a file path target — so the claim that a file target is rendered with the directory form is pinned here rather than only by the evidence goldens. These tests are written before the renderers: the grammar is fully specified above.
