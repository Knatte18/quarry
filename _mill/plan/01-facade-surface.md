...
- **Requirements:**
  ...
  In `quarry/text_test.go`, add a table per renderer covering every branch listed above as an exact expected string, and assert the no-trailing-whitespace-and-one-newline property with the file's existing `assertNoTrailingWhitespaceAndOneNewline` helper. These tests are written before the renderers: the grammar is fully specified above.
