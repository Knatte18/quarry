...
- **Requirements:**
  ...
  In `quarry/text_test.go`, add a table per renderer covering every branch listed above as an exact expected string, and assert the no-trailing-whitespace-and-one-newline property with the file's existing `assertNoTrailingWhitespaceAndOneNewline` helper. Include all three totality guards in that table, each driven by a value an external caller could construct: the expand renderer's fall-through branch from a zero answer value, its found branch's nil-head guard from a found answer whose head pointer is nil, and the resolve renderer's nil-directory-answer guard from a found path result whose pointer is nil. The one-trailing-newline promise is pinned for each. Include a path-target found case whose directory answer holds exactly one file entry — the shape the engine produces for a file path target — so the claim that a file target is rendered with the directory form is pinned here rather than only by the evidence goldens. These tests are written before the renderers: the grammar is fully specified above.
