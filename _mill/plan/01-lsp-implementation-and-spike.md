...
- **Moves:** none
- **Requirements:**
  - Add a field `ImplementationProvider capabilityFlag` with json tag `implementationProvider` to the `capabilities` struct in `internal/quarryengine/lsp/lspclient.go`, alongside the existing `WorkspaceSymbolProvider` and `DocumentSymbolProvider` fields, and widen that struct's own doc comment in the same edit — it currently says the struct "reports the server's workspace/symbol and documentSymbol support", which is incomplete the moment a third capability joins it. The existing `capabilityFlag.UnmarshalJSON` already normalizes the bool-or-object shape; no change is needed there.
...
- **Requirements:**
  ...
  - The test reuses the package-level `repoRoot` helper that `internal/quarryengine/query/refs_integration_test.go` already declares under the same `lsp` build tag — call it, do not redeclare it, or package `query` fails to build with a duplicate declaration under that tag. The same applies to `findFuncPosition` wherever the position search can use it.
  - The test resolves the fixture root that way, spawns a gopls client with `lsp.NewClient`, initializes it rooted at the fixture module directory, and then, at each of two positions, issues `textDocument/definition`, `textDocument/implementation`, and `textDocument/references` and logs every returned location with `t.Logf`. The two positions are (a) the `Now` method inside the `clock` interface declaration in package `builder`, and (b) the `Now` method on the concrete `realClock` type in package `builder`.
  - At the interface-method position only, additionally issue one `textDocument/definition` per location the `textDocument/references` call returned, and log each reference alongside the definition result it resolved to. Without this the spike's output cannot be filtered even by hand: the verification filter consumes exactly that per-reference definition set, so card 4's counts are underivable from the three position-level calls alone.
...
- **Requirements:**
  ...
  - Also record three counts measured from the spike's own output at the interface-method position, each under its own literal label so a later card can cite one figure unambiguously. Label them `references-unfiltered:` (the total number of references returned), `references-verified:` (how many remain after applying the recorded widening mode's filter by hand — keep a reference when its own logged per-reference definition result shares a location with the declaration match set the mode selects, and drop it only when that result is non-empty and shares none), and `callers-verified:` (`references-verified` minus the declaration sites the position-level definition result names, which is what `assert-no-callers` actually reports, since its `callers` list excludes every returned declaration).
