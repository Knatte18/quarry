...
- **Requirements:**
  ...
  - Also record two counts measured from the spike's own `textDocument/references` output at the interface-method position: the total number of references returned unfiltered, and the number that remain after applying the recorded widening mode's filter by hand against the logged definition and implementation results. These two numbers are what batch 6's live test asserts against. Do not carry issue #1's "31 → 2" figure forward — that is a pre-fix measurement taken on a different repo.
