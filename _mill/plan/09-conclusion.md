...
- **Requirements:** Synthesise the matrix into prose, sourcing every number from the tracked summary and never from a run's raw artifacts or from memory of watching the runs. Required content:
  ...
  - **Warm versus cold** — the `a5-bundle` against `a5-bundle-cold` contrast under the same disjoint-range bar, or, when the cold cell was reported as not run, a statement that the supervised strategy was unavailable and the warm-vs-cold question remains open, with no numbers offered in its place.
