...
- **Requirements:** `golden_test.go` runs two cases through `Open` on the Loomyard checkout and
  ...
  **Producing the goldens needs the checkout, and the card says what to do without one.** The two
  committed goldens can only be generated by running `-update` with `LADDER_LOOMYARD_REPO` naming a
  checkout at the pin; the per-machine value lives in the gitignored `.scratch/ladder.env` the
  previous card documents. Source that file and generate them. If this machine has no such
  checkout, or it is not at the pin, the card is **blocked** and must be reported as blocked —
  never satisfied by committing hand-written or invented goldens, which would pin the wrong bytes
  and pass forever. The comparison cases themselves skip with the rest of the Loomyard suite when
  the gate skips, so an unproduced golden fails loudly at generation time rather than silently at
  comparison time.
...
- **Requirements:** The headline criterion, over this repository, with no environment needed so it
  ...
  way `SpansOf` builds its own — `newIgnoreSet` on the repository root plus one `extend(".")` and
  nothing more, with `symbolsOfUnit` owning every step below the root; then assert, per glyph, that the set of
