...
- Decision: each session runs from a disposable scratch directory derived from `ladder.yaml`'s `session_dir_template` (e.g. `/tmp/ladder-session-a5-bundle-1`). The `ladderbench` binary and the results root are addressed by absolute path. The generated `.mcp.json`, `settings.json`, and both agent definitions live in that scratch dir; the skill does not (see the skill-delivery decision above).
...
- **The `skill_listing` channel.** Every subagent transcript carries a `skill_listing` record enumerating the session's available skills by **name and description, verbatim**, with no tool call involved — verified in a real subagent transcript. That makes user-scope skill descriptions a leak channel into a blinded run agent's transcript that no cwd hygiene can close. Two mitigations, both mechanical:
  ...
  2. `prepare-session` scans every `~/.claude/skills/*/SKILL.md` frontmatter for a case-insensitive "quarry" and, **for a `none` config only**, hard-fails naming the offending skill so the operator relocates it before launching a blinded session. This is a precondition in the same family as the environment-variable check.
...
- Blinding-hygiene generators: neither generated agent definition for a `none` config contains a case-insensitive "quarry"; the tracked `ladder-run` `SKILL.md`'s frontmatter contains no case-insensitive "quarry"; the `~/.claude/skills/` precondition scan fails on a quarry-mentioning frontmatter for a `none` config and passes for a rung.
...
- **Q:** The parent session transcript carries the scorer prompt, which embeds the unstripped fasit. **A:** [auto-pick] A `none` config gets one session per repetition and its scoring is deferred to a separate scoring session; total sessions rise to 23. **Why:** otherwise rep 1's fasit sits within `Read` range of reps 2 and 3 of the same blinded config — the answer key, not a marginal leak.
