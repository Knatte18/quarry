...
- Residual limit, stated plainly: the operator's user-scope skills are outside this harness's control between the precondition check and the launch, and `~/.claude/skills/` remains reachable by `Bash` from any cwd. These are hygiene measures that close the two channels the harness itself creates, not a structural guarantee — the same class of limit `ladder/README.md` already acknowledges for the `none` arms generally. `gate_blinding` scans the whole transcript, `skill_listing` records included, and remains the detector.
