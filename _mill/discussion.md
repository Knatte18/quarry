...
The `quarry/` directory at the module root holds the entire engine as one flat Go package: 14 production files (2,742 LOC) plus 20 test files (4,267 LOC). Those 14 files mix two clearly distinct concerns — LSP daemon lifecycle (`registry.go`, `ensureserver.go`, `lspclient.go`, `toolchain.go`, `daemonstate.go`, `probe.go`) and the query verbs (`definition.go`, `refs.go`, `symbol.go`, `detect.go`, `position.go`, `load.go`) — with `errors.go` and a 289-line `doc.go` on top.
...
Why now: no external forcing function — this is deliberate structural cleanup taken while the engine's public surface is still small (29 exported top-level identifiers) and before `cmd/quarry-mcp` lands and doubles the number of consumers that would have to be migrated later. A second, smaller reason rides along: every error and log message in the engine still carries a `scoutengine: ` prefix inherited from the Loomyard port, naming a package that no longer exists anywhere in this repo.

## Scope

**In:**

- Move all 14 production files out of `quarry/` into a five-package DAG under `internal/quarryengine/` (layout fixed below under Decisions → *Package layout*).
...
- Move all 20 test files alongside the production files they cover, keeping them in-package (white-box), splitting any file whose subtests straddle a new package boundary.
...
- Export the `toolchain.go` test seams (`daemon.InstallGoToolchain`, `daemon.UserCacheDir`, `daemon.ToolchainInstaller`) and add the test-support package `internal/quarryengine/daemon/daemontest`, so `query`'s `refs_test.go` can still drive them (see Decisions → *toolchain test seam*).
...
- Replace every `scoutengine: ` prefix in engine error and log messages with `quarry: ` (59 occurrences across 9 files), and update `docs/port-equivalence.md`'s note that documents the old prefix.
...
- **Decision:** the engine becomes a five-package DAG:
  ...
  Allowed import directions: `quarryengine` imports no subpackage; `lsp` and `registry` import `quarryengine` only; `daemon` imports `quarryengine`, `registry`, `lsp`; `query` imports all four. A sixth, test-support-only package `internal/quarryengine/daemon/daemontest` (see the *toolchain test seam* Decision) imports `daemon` and is imported only from `_test.go` files.

- **Rationale:** the literal daemon/query split named in the task brief does not compile. Measured cross-file dependencies (comments stripped) show a mutual dependency between the two proposed halves: `lspclient.go` (daemon half) uses `lspPosition` ×2, `lspRange` ×4, `lspLocation` ×10, and `formatLocation` ×2 — all declared in `position.go` (query half). Both halves also depend on `errors.go` — the daemon half constructs or matches `ErrServerNotFound`, `ErrServerSpawnTimeout`, `ErrServerTimeout`, `ErrServerTimeoutSentinel`; the query half additionally uses `ErrAmbiguousSymbol`, `ErrResolverUnsupported`, `ErrSymbolNotFound`, `ErrNoLanguage` (the qualitative claim is what matters here, not a headcount) — and `lspclient.go` uses `defaultLogHandler`, which is declared in `ensureserver.go`. A shared leaf package is therefore forced, not optional — this settles the brief's step-7 open question in favour of "yes, a shared package", though it lands as the `quarryengine` root package rather than a sibling named `shared`.
...
- **Decision:** `quarry/` re-exports exactly the 29 top-level identifiers it exports today and nothing more:
...
- **Decision:** add `internal/quarryengine/layering_test.go` asserting the allowed import directions from the *Package layout* table above: the root package imports no `internal/quarryengine/...` path; `lsp` and `registry` import at most the root; `daemon` imports at most root + `registry` + `lsp`; `query` may import all four. Its walk is **not** the seam test's: the seam test visits non-test `.go` files only (a banned third-party/CLI import matters when it ships in production code), whereas `layering_test.go` visits **both** production and `_test.go` files, because a test file importing across the DAG is exactly how the layering rots in practice. The two share the directory-walk helper, not the file filter. There is precisely one carve-out: any `_test.go` file may import `internal/quarryengine/daemon/daemontest` regardless of its own package's layer — that is the whole reason `daemontest` exists (see the *toolchain test seam* Decision). Nothing else is exempt: `registry`'s own `_test.go` importing `daemon` **is** a violation and must fail the guard, and so is a `query` test importing `daemon` directly rather than through `daemontest`.
...
- **Decision:** the 289-line `quarry/doc.go` splits into: `internal/quarryengine/doc.go` carrying the architecture overview (the engine/CLI split, the typed error vocabulary, the scope boundaries — rewritten to describe the five-package layout and to name each package's role); a package doc comment hosted in a named file per subpackage — `lsp/lspclient.go`, `registry/registry.go`, `daemon/ensureserver.go`, `query/refs.go` — covering that package's own section (`lsp` ← "The generalized LSP client" minus its resolver paragraphs, plus the position-conversion paragraph; `registry` ← "The language-server registry"; `daemon` ← "The EnsureServer seam" + "Go toolchain manager" + "Daemon state and concurrency"; `query` ← the `workspace/symbol` resolver, `--in-file` resolver, and ambiguity paragraphs). A dedicated `doc.go` in a subpackage is an acceptable substitute where the comment would otherwise dwarf the file it heads — `daemon`'s is the likely case, since it inherits three sections; and a new ~20-line `quarry/doc.go` stating that this package is a stable thin re-export of `internal/quarryengine`, that it adds no behaviour, and pointing at the engine's own docs.
- **Rationale:** the existing text is accurate and expensive to reproduce — it should be relocated section by section, not rewritten from scratch. Most sections map onto exactly one of the new packages, which is corroboration that the boundaries are the real ones. One does not, and the split is paragraph-level there rather than section-level: "The generalized LSP client" (`quarry/doc.go` L38–98) opens with the eight-method wire description and the per-phase timeout rules, which belong to `lsp`, but its middle (L65–87) is the `workspace/symbol` resolver, the `--in-file`/`documentSymbol` resolver, and the zero/one/many → `ErrSymbolNotFound`/success/`ErrAmbiguousSymbol` mapping — all of which describe `resolvePosition` in `refs.go` and therefore belong to `query`. Split that one section mid-body; the rest move whole. References inside the moved prose to files and unexported identifiers (`ensureServer`, `lspClient`, `close()`, `kill()`, `resolvePosition`) must be updated to the new package-qualified, exported names.
...
- **Decision:** replace the literal `scoutengine: ` prefix with `quarry: ` in every engine error and log message — 59 occurrences across `errors.go` (12), `ensureserver.go` (18), `lspclient.go` (8), `daemonstate.go` (6), `toolchain.go` (5), `registry.go` (4), `detect.go` (2), `load.go` (2), `refs.go` (2). Also correct the live claim in `docs/port-equivalence.md` (~lines 105–106) that documents quarry's errors as still carrying the old prefix, including the sample JSON error string quoted there. Two outliers in the same file are folded into the same pass. First, `errors.go:160` — `(*ErrServerSpawnTimeout).Error()` — carries a **`scout: `** prefix, not `scoutengine: `; it is the only such occurrence in the repo, it is excluded from the 59 count, and it is renamed to `quarry: ` too, so all six error types end up with one consistent prefix. Second, `errors.go:22` carries a stale doc-comment reference to `scoutengine.ErrServerNotFoundSentinel` — a package-qualified identifier, not a message prefix, which is why it is excluded from the 59 count; it is renamed to `quarryengine.ErrServerNotFoundSentinel` in the same pass so the acceptance grep in Testing can come back clean.
...
- **Decision:** every `_test.go` file keeps its `package <pkg>` (in-package) form and moves to the package holding the code it covers; identifiers it references that are now exported get the mechanical rename. Placement:
  ...
  - new, not a move: `internal/quarryengine/daemon/daemontest/daemontest.go` (test-support helpers, no production code)
...
### The toolchain test seam is exported, and its helpers move to a `daemontest` package

- **Decision:** the two package-level test seams in `toolchain.go` become exported vars on the `daemon` package — `daemon.InstallGoToolchain` (type `daemon.ToolchainInstaller`, also exported) and `daemon.UserCacheDir` — each documented at its declaration as an injection point that exists for tests. A new test-support package `internal/quarryengine/daemon/daemontest` exports `WithFakeInstaller(t *testing.T, fake daemon.ToolchainInstaller)` and `WithTempUserCacheDir(t *testing.T) string`, both restoring the previous value via `t.Cleanup`, lifted verbatim from `toolchain_test.go:22` and `toolchain_test.go:33`. The local copies in `toolchain_test.go` are deleted; `toolchain_test.go`, `ensureserver_test.go` (in `daemon/`) and `refs_test.go` (in `query/`) all call the `daemontest` helpers instead. `daemontest` contains no production code and is imported only from `_test.go` files.

- **Rationale:** `refs_test.go`'s `TestReferences_HasNativeDaemonRoutesThroughEnsureServer` calls `withTempUserCacheDir(t)` and `withFakeInstaller(t, …)` at `refs_test.go:391,394`, and those helpers reassign identifiers that are unexported in `toolchain.go`. Once `refs_test.go` lands in `query/` and `toolchain.go` in `daemon/`, that subtest cannot compile — it is the one place the split genuinely breaks a test, and it has to be settled here rather than discovered mid-implementation.

  Every alternative is worse for a structural reason, not a taste one. The subtest cannot move into `daemon/` as an in-package test, because it exercises `query.References` and `query` already imports `daemon` — an import cycle. It cannot move into a `package daemon_test` external test file either: that resolves the cycle but still cannot reach `daemon`'s unexported vars, which is the actual blocker. It cannot be duplicated into `query/` for the same reason. So the seam must become reachable across a package boundary, which means exported. Given that, routing every caller through `daemontest` rather than poking the vars directly keeps the save/restore discipline in exactly one place and keeps the exported surface to the two vars plus one type.

- **Rejected:** deleting or skipping the subtest (it is the only test proving `HasNativeDaemon` routes `References` through `EnsureServer` — the central V1 dispatch claim); changing `EnsureServer`'s signature to take an injectable installer (a production API change inside a reorganization, and a wider blast radius than two exported vars); `export_test.go` in `daemon` (Go's `export_test.go` mechanism is visible only to that package's own test binary, so it cannot serve `query`'s tests at all).

- **Consequence for the layering guard:** `daemontest` is a sixth package under `internal/quarryengine/`. It sits outside the production DAG: it may import `daemon`, and it is the single carve-out in `layering_test.go`'s test-file scan defined in the *layering guard* Decision above — any package's `_test.go` may import it, and no other cross-layer test import is exempt. The seam test still scans `daemontest` for the banned imports like any other package. Both guards' expected-package-count assertions include it.
...
- **Rejected:** no facade test (the failure mode is silent and lands in a different package); a reflect-based surface-diff test (over-built for 29 identifiers, and the compiler already does the work).
...
- **Identifiers that must become exported** (measured, comments stripped — these are the query-half → daemon-half edges plus the wire types):
  ...
  - from `ensureserver.go` → root: `defaultLogHandler` moves to `internal/quarryengine/log.go` and is exported as **`quarryengine.Logger`**, a package-level `*slog.Logger` — that is the name, not one of two candidates. `lspclient.go` and `ensureserver.go` are its only users (5 + 2 call sites).
...
- **One test seam does cross a package boundary — see the `daemontest` Decision.** The package-level vars reassigned by tests are `installGoToolchain` and `userCacheDir` (both `toolchain.go`). They are reassigned only through the helpers `withFakeInstaller`/`withTempUserCacheDir`, declared in `toolchain_test.go:22` and `toolchain_test.go:33`. Three test files call those helpers: `toolchain_test.go` and `ensureserver_test.go:336,342` (both land in `daemon/`, fine) **and `refs_test.go:391,394`** — `TestReferences_HasNativeDaemonRoutesThroughEnsureServer`, which lands in `query/` and would no longer compile. `toolchain_integration_test.go:28` assigns `userCacheDir` directly and stays inside `daemon/`. `precedence` and `defaultLogHandler` are never reassigned by tests.
...
- **Message-prefix rename** — `grep -rnE 'scoutengine|"scout: ' --include='*.go' .` must return only `refs_integration_test.go:55`'s historical comment afterwards. The alternation is required: `errors.go:160`'s outlier is `"scout: "`, which a bare `scoutengine` grep silently misses. Check whether any `internal/cli` test asserts on a full engine error string; if one does, update it as part of the rename rather than as a separate step.
...
- **Q:** What exactly does the facade re-export, and as aliases or wrappers? **A:** [auto-pick] Exactly today's 29 exported top-level identifiers; types as `=` aliases, sentinels as re-export vars, functions as delegating wrappers. **Why:** `internal/cli` runs `errors.As`/type assertions against `quarry.ErrAmbiguousSymbol` and `quarry.ErrSymbolNotFound`; a defined type or a fresh `errors.New` would compile in `quarry/` and silently stop matching at the CLI call site.
...
- **Q:** `refs_test.go` (destined for `query/`) calls `withFakeInstaller`/`withTempUserCacheDir`, which reassign unexported vars in `toolchain.go` (destined for `daemon/`) — the split breaks that subtest. What is the disposition? **A:** [auto-pick] Export the two seams as `daemon.InstallGoToolchain` / `daemon.UserCacheDir` (plus the `daemon.ToolchainInstaller` type) and move the helpers into a new test-support package `internal/quarryengine/daemon/daemontest`, which all three calling test files then import. **Why:** the subtest proves the central V1 claim that `HasNativeDaemon` routes `References` through `EnsureServer`, so deleting it is not an option; and it cannot be relocated — an in-package `daemon` test importing `query` is an import cycle, and an external `daemon_test`/`query_test` package still cannot reach unexported vars. Making the seam reachable across a package boundary is therefore forced, and `daemontest` keeps the save/restore discipline in one place instead of scattering it across three test files.
- **Q:** The `scoutengine: ` prefix on every engine error and log message names a package that no longer exists. In scope or a follow-up? **A:** **In scope — remove it** (operator decision, overriding the auto-pick, which had deferred it as an observable-behaviour change). Replace with `quarry: ` across all 59 occurrences in 9 production files, and fix `docs/port-equivalence.md`'s live claim about the old prefix. **Why:** the operator explicitly pulled it into this task; `quarry: ` rather than no prefix, because the marker still identifies the error's origin once `internal/cli` wraps it into the JSON envelope, and `quarry` is the name of both the module and the binary. This is the single intentional behaviour change in an otherwise pure reorganization and is called out as such under Scope.
