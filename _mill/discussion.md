...
- **Consequence for the layering guard:** `daemontest` is a sixth package under `internal/quarryengine/`. The layering test must allow `daemontest` → `daemon` and must not treat a `_test.go`-only importer of `daemontest` as a layering violation; the seam test must still scan `daemontest` for the banned imports like any other package. Both guards' expected-package-count assertions include it.
