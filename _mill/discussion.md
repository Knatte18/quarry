...
- Two new `ladder.yaml` fields (`run_effort`, `session_dir_template`), and blanking `max_turns` to `null` because its accounting basis changed.
- A new `.gitignore` entry `bench/loomyard-eval/ladder/results/**/.session-active`, beside the existing `results/**/raw/` rule.
...
- **Running the 45-run matrix.** This task delivers the harness only; matrix execution and `conclusion.md` are a separate follow-up task. Neither preflight probe is dispatched, and no run of any config is made. The one exception is the **smoke launch** described below, which is in scope and produces no results-tree artifact.
...
- Decision: **one session per repetition, for every config, running exactly one run agent and no scorer** — plus **one scoring session for the whole matrix**. 45 run sessions + 1 scoring session = **46 sessions**. Launched by `ladderbench prepare-session --config-id <id> --rep <n>`, and `ladderbench prepare-session --scoring` for the scoring session.
- Arithmetic, matching `ladder.yaml` and `plan_runs`: 15 configs × 3 repetitions = 45 runs, of which 42 (the 14 non-cold configs) are the main matrix and 3 (the cold config) are the cold cell. One run session each.
- Two things force this rather than one session per config. **`settings.json` is one file per session**, so a session hosting several configs can carry only one deny-list, and the two-layer enforcement model below is only genuinely per-rung when the session is per-config. **The session transcript is a shared surface**, so a session hosting several repetitions puts each earlier repetition's answer — and, if it scored inline, the task's unstripped fasit — within `Read` range of the later ones. See the blinding-channel decision below.
- Separating scoring entirely is what closes the fasit channel: no session ever contains both a run agent and a scoring prompt. The single scoring session is safe to share across the whole matrix precisely because **the scorer agent definition grants zero tools** — a scorer cannot read the session transcript that accumulates the other runs' fasits, or anything else.
- The cost is real and accepted: 46 supervised launches. Each run session is exactly one run, which is also the supervision unit the whole dispatch swap exists to provide — a rogue run is killed by closing the one session it is in.
- Rejected: per-ladder sessions with the deny-list reduced to `Task` only (drops the backup layer, and the second preflight probe would then be testing a layer that does not exist); per-ladder sessions with a per-rung deny-list written but unenforceable (a check that cannot fire reads as coverage that is not there); one session per config running all 3 repetitions (the transcript-sharing problem above — and applying the per-repetition split to blinded configs alone would have conflated tool-provenance blinding with answer-key exposure, when a rung's inflated score is the primary measured outcome and `gate_blinding` does not even run for a rung).

### The session's working directory is a neutral scratch dir

- Decision: each session runs from a disposable scratch directory derived from `ladder.yaml`'s `session_dir_template` (e.g. `/tmp/ladder-session-a5-bundle-1`). The `ladderbench` binary and the results root are addressed by absolute path. A run session's scratch dir holds the generated `.mcp.json` (config permitting), `settings.json`, and the **run agent definition only**; the scoring session's holds `settings.json` and the **scorer definition only**. Neither carries the other's, and neither carries the skill (see the skill-delivery decision above).
...
- **What a `none` run session's scratch directory may contain — exhaustively.** The blinded run agent has `Read`/`Grep`/`Glob`/`Bash` and its cwd *is* the scratch dir, so anything written there is one `cat` away from the transcript, and `gate_blinding` makes any `mcp__quarry__` substring anywhere in the transcript fatal. A `none` run session's scratch dir contains exactly two things and nothing else: the run agent definition (allowlist `Read`/`Grep`/`Glob`/`Bash`, naming nothing) and a `settings.json` whose `permissions.deny` is **`["Task"]` and nothing else**. No `.mcp.json`, and **no scorer definition** — a run session dispatches no scorer, so writing one would put an unused file in the blinded agent's cwd for nothing. The run agent definition's name, description, and body are constrained to be free of any case-insensitive "quarry" or `mcp__quarry__` substring, enforced by a unit test over the generator.
- Generally: `prepare-session` writes the scorer definition **only into the scoring session's scratch dir**, and the run agent definition only into run sessions' scratch dirs. Neither session type carries the other's.
...
- **The `skill_listing` channel.** Every subagent transcript carries a `skill_listing` record enumerating the session's available skills by **name and description, verbatim**, with no tool call involved — verified in a real subagent transcript. That makes user-scope skill descriptions a leak channel into a blinded run agent's transcript that no cwd hygiene can close. Two mitigations, both mechanical:
  1. The `ladder-run` skill's own frontmatter name and description are constrained to contain no case-insensitive "quarry" (e.g. "Drive one benchmark session's dispatch, ingest, and score loop"). A unit test over the tracked `SKILL.md` asserts this.
  2. `prepare-session` scans **every discoverable skill source**, not just one, and **for a `none` config only** hard-fails naming the offending skill so the operator relocates it before launching a blinded session. The roots are `~/.claude/skills/*/SKILL.md` (user scope) **and** `~/.claude/plugins/cache/*/*/*/skills/*/SKILL.md` (plugin caches, which is where installed skills actually live on this machine — `~/.claude/skills/` does not currently exist, so scanning it alone would pass vacuously and close nothing). The session's own project scope is the scratch dir, which by construction holds no skills. A root that does not exist is skipped rather than erroring, but `prepare-session` prints every root it scanned and the file count found at each, so a vacuous pass is visible rather than silent. This is a precondition in the same family as the environment-variable check.
- **The parent session transcript is a third channel, and it carries the fasit.** The orchestrating session writes its own `~/.claude/projects/-tmp-ladder-session-<id>/<session-id>.jsonl`, which accumulates every `ladderbench` invocation, every dispatched agent's returned answer, and — critically — the scorer prompt `redact` prints, which embeds the task's **unstripped fasit**. A session running several repetitions with scoring inline would put each earlier repetition's answer and its task's fasit within `Read` range of the later ones.
- **This is not a blinding problem, it is an answer-key problem, and it applies to every arm.** Tool-provenance blinding and fasit exposure are different things: `gate_blinding` runs only when `config.allowed` is empty, so nothing would detect a rung reading a fasit — and a rung's recall/precision is the primary measured outcome the whole ladder exists to compare. Closing this for the blinded arms alone would have been the worse half of the fix.
- **Therefore the session model is uniform: one session per repetition, one run agent, no scoring; and all scoring happens in one dedicated scoring session.** No session ever contains both a run agent and a scoring prompt, for any config. The single scoring session is safe to share across all 45 runs because the scorer agent definition grants **zero tools** — a scorer cannot read the transcript accumulating the other fasits, or anything else on disk.
- Residual limit, stated plainly: a run session's transcript still accumulates that session's own `ladderbench` command lines, which name the results root inside `repo_root`. A blinded agent that went looking for its own session transcript would find one — and `gate_blinding`'s repo-root check would fire fatally and invalidate the run, which is the correct outcome rather than a silent leak. A rung is not scanned by that gate, but a rung reading its own command lines learns only the results path, never a fasit. Likewise the operator's user-scope skills are outside this harness's control between the precondition check and the launch, and `~/.claude/skills/` remains reachable by `Bash` from any cwd. These are hygiene measures that close the channels the harness itself creates, not a structural guarantee — the same class of limit `ladder/README.md` already acknowledges for the `none` arms generally. `gate_blinding` scans the whole transcript, `skill_listing` records included, and remains the detector.
...
- Decision: `usage.json` is rebuilt around the subagent transcript. The partition below is exhaustive over every field `extract_usage` writes today.
  ...
  - **Added:** `effort`, `agent_id`, `transcript_source` (the original `~/.claude/projects/.../agent-<id>.jsonl` path the run-directory `transcript` was copied from), and `denied_tool_attempts_provisional` (see the provisional-marker decision below).
...
- Consequence: `summarize`'s `METRICS` tuple loses `wall_clock_ms` and `cost_usd`; every remaining entry keeps its definition. One shape change to `summary.json`: a cell's `denied_tool_attempts` stats record carries the provisional marker through from the runs it summarised.
...
- Decision: a **run session** covers exactly one repetition, and `warm` / `restore-worktree` sit **inside the attempt loop**, exactly where `run_matrix` puts them. In full:

  ```
  # run session: prepare-session --config-id <id> --rep <n>
  for attempt k in 1..MAX_ATTEMPTS:
    warm                            # skipped entirely for a cold config
    dispatch run agent              # description: "ladder <config-id> rep <n> attempt <k>"
    ingest                          # takes observe_worktree_dirtied, then writes ingest.json
    restore-worktree                # unconditional, every attempt, success or not
    if truncated:  halt the matrix
    if failed:     invalidate; next attempt
    break
  prepare-session --release
  ```

  ```
  # scoring session: prepare-session --scoring, run once after every run session
  for each run dir with an ingest.json and no run.json, in (config, rep) order:
    redact -> dispatch scorer agent -> record-score
  prepare-session --release
  ```

- **`ingest.json` is the new intermediate marker, and it is what makes the split work.** `run.json` is still written last, by `record-score`, and `gates.is_complete` still keys on it — that definition is untouched, and `summarize` / `load_runs` still read it as the sole definition of a complete run. But with scoring deferred to a later session, `run.json` cannot also serve as the run session's progress marker. So `ingest` writes `<run_dir>/ingest.json` on success, recording the config id, repetition, attempt, and the gate report's non-fatal observations. `next-run` filters run sessions on `ingest.json`, the scoring session filters on "`ingest.json` present, `run.json` absent", and `invalidate` removes `ingest.json` along with the rest of the directory it renames aside.
- The single-flight predicate is restated against that marker: `ingest` for repetition `n` fails when repetition `n-1` of the same config has neither an `ingest.json`, a `run.json`, nor an exhausted attempt record. It therefore holds across sessions, not merely within one, since the marker is on disk.

- Why `restore-worktree` is unconditional and per attempt: an agent that edited files during a failed attempt would otherwise leave them in place for the retry, and `observe_worktree_dirtied` would attribute one attempt's dirtying to the next. `ingest` runs the gates — and therefore takes the dirtied observation — before the restore, because the restore is precisely what erases the evidence.
- Why `warm` is per attempt: the daemon self-expires after its idle timeout, so a retry that skipped the warm-up could dispatch against a cold daemon and silently contaminate the warm arm's timings.
- Rationale: one repetition per run session makes the operator's supervision unit exactly one run, which is the point of the dispatch swap, and it is what keeps each run's session transcript free of any earlier run's answer. Scoring is separated for the fasit reason given under the tool-exposure decision, not for ordering reasons.
...
- **The threshold must be re-baselined, so this task blanks it.** The committed `max_turns: 60` was calibrated against the `claude -p` client's own `--max-turns` / `result.num_turns` accounting. The gate's basis is now "assistant records in a subagent transcript", which is a different count, and nothing establishes that 60 means the same thing on it. `ladder.yaml`'s `max_turns` therefore ships `null` — exactly as `run_model` already does — and `require_pins` continues to reject an unset value, so the operator must set it before the matrix starts, informed by the follow-up task's first real runs. The field's documented meaning changes to "maximum number of assistant records in a run's subagent transcript".
- Rationale for blanking rather than keeping 60: a threshold carried over from a different accounting basis is either effectively dead or spuriously fatal, and there is no way to tell which without a run. That is the dead-check problem again — a number that reads as calibrated when it is not.
...
- **Exactly one match whose sibling `.jsonl` is absent is its own case, and it is not hypothetical** — subagent directories holding `.meta.json` files with no sibling transcript exist on this machine. `ingest` treats it as *possibly not yet flushed*: it re-checks on a short bounded wait, then hard-errors naming the meta path and the description it matched. It never falls back to another file, and it never proceeds with a partial custody chain — the whole point of matching on description is that the transcript it copies is provably the right one.
...
- **`redact_text` keeps its `/tmp/quarry-bench` alternation branch**, even though the gate check is dropped. A gate that cannot fire is worse than no gate because it reads as coverage; a redaction branch that never matches removes nothing and asserts nothing — a redactor's job is to over-remove, and an answer quoting sibling-suite prose or an operator-supplied path is not impossible. The two dispositions differ because the two mechanisms make different claims.
- Rejected: keeping the dead `/tmp/quarry-bench` gate check; dropping the redaction branch alongside it (conflates an assertion with a scrub); dropping `gate_blinding` on the grounds that the allowlist makes exposure structurally impossible.
...
- Decision: one Agent call per run, all of them in the single scoring session. The scorer uses one generated agent definition — identical for every config, since only `scorer.model` and `scorer.effort` parameterise it — pinned to the `opus` alias at `high` effort with **no tools at all**. The three-input blinded contract (redacted answer, `_meta`-stripped fasit, task text) is unchanged. `ladderbench redact` prints the scorer prompt; the session dispatches it; `ladderbench record-score` consumes the reply.
- The scoring session is provisioned by `prepare-session --scoring`: scratch dir from `session_dir_template` with `{config_id}` = `scoring` and `{n}` = `1`, containing the one scorer definition and a `settings.json` denying `Task`. No `.mcp.json`, no run agent definition, no worktree setup, no `quarry-mcp` build. It takes the `.session-active` lockfile like any other session and calls the full `require_pins`, since `record-score` stamps `scorer.model` / `scorer.effort` into every `score.json`.
...
- Ownership of every cold-cell step, so none is left without a subcommand:
  ...
  - `cold-cell` with no flags, run once after all cold sessions, finalises `cold_cell.json`.
...
- **A failed cold attempt relaunches the whole session; there is no in-session cold retry.** The session's `quarry-mcp` process lives for the session's lifetime, so its daemon survives across attempts within one session. Re-running `clear_state_dir` mid-session would delete the `daemon.json` that `gate_cold_before` reads the pid from, leaving a live warm daemon invisible to the precondition — attempt 2 would then be *reported* cold while running against a daemon attempt 1 started. That is exactly the failure the cold cell exists to rule out.
- So a cold attempt's failure path is: `invalidate`, `cold-cell --teardown --rep <n>`, session exits, operator re-runs `prepare-session --config-id a5-bundle-cold --rep <n>` for attempt `k+1`. `MAX_ATTEMPTS = 3` still caps it, now counted in sessions rather than in-session iterations, and `next-run` still derives the attempt index from the `<n>.invalid-<k>` siblings on disk, which survive the relaunch.
- Rejected: an in-session `cold-retry` subcommand redoing remove/build/clear (cannot make the surviving server's daemon go away); keying `gate_cold_before` on something other than the state file's pid (that pid is exactly what quarry's own staleness check reads, and diverging from it would make the gate measure something quarry does not).
...
- Decision, **within a session — mechanically enforced**: the skill mandates exactly one Agent call in flight at a time, and `ladderbench ingest` fails loud when it is asked to ingest repetition `n` while repetition `n-1` of the same config has neither a `run.json` nor an exhausted attempt record — a predicate the per-repetition session loop above satisfies by construction, and which a batched "all runs then all scores" order would violate immediately.
- The lockfile lives at `<results_root>/.session-active` and is **gitignored** by a new `bench/loomyard-eval/ladder/results/**/.session-active` entry, alongside the existing `results/**/raw/` rule — it is live session state, never part of a committed result.
- Decision, **across sessions — a cheap mechanical guard over operator discipline**: `ingest`'s predicate cannot see another session, and cross-session concurrency is exactly the contention `run_matrix`'s docstring says makes wall-clock incomparable. So `prepare-session` writes `<results_root>/.session-active` naming the config it prepared and refuses to prepare a second session while one exists; `prepare-session --release` clears it, and the skill's final step in every session calls it. This is a guard, not a proof — an operator who deletes the file or launches from a second checkout defeats it — so the README states cross-session serialization as an operator obligation, with the lockfile as the thing that catches the ordinary mistake.
- Rejected: leaving cross-session serialization as prose alone (the ordinary mistake — forgetting a session is still open — goes undetected); a pid-based lock (a session outlives any `ladderbench` process, so no live pid represents it).
...
- Decision: add `run_effort: medium` and `session_dir_template: /tmp/ladder-session-{config_id}-{n}`. `require_pins` extends to reject an unset `run_effort` alongside `run_model`, `max_turns`, `scorer.model`, and `scorer.effort`. `run_model` continues to ship `null` by design, and `max_turns` is **changed to ship `null`** for the reason given in the `max_turns` decision — two operator-supplied pins now, not one.
- **Which subcommands enforce `require_pins`.** Every subcommand whose output depends on a pinned value calls it in full: `next-run`, `warm`, `ingest`, `redact`, `record-score`, `cold-cell`, and `summarize`. `prepare-session` does **not**: it needs only `run_model` (baked into the generated agent definition), `run_effort`, `session_dir_template`, and `scorer.model` / `scorer.effort`, all of which except `run_model` ship with committed values — so it enforces that narrower set and leaves `max_turns` alone, since the ceiling is a gate-time value nothing about session preparation touches.
- **How the smoke launch satisfies it.** `prepare-session` accepts `--run-model <id>` as an override used only by the smoke launch; it is never written back to `ladder.yaml`. With it, the smoke launch runs against the committed file while `run_model` and `max_turns` are both still `null`, and no other subcommand is invoked during it.
- `{n}` is the **repetition index**, uniformly, and `--rep <n>` is **required** for every run session of every config. Every one of the 45 run sessions therefore gets its own scratch dir, which is what "each session runs from a disposable scratch directory" and `ingest`'s one-directory transcript search space both depend on.
- The scoring session uses `{config_id}` = `scoring` and `{n}` = `1`; the two probe sessions use `{config_id}` = `probe-allowlist` / `probe-denylist` and `{n}` = `1`.
...
The project directory name is the session's cwd with path separators replaced by `-` (e.g. `/tmp/ladder-session-a5-bundle-1` → `-tmp-ladder-session-a5-bundle-1`).
...
- `.claude/agents/<config-id>.md` — the run agent definition. Written **only** for a run session; the scoring session gets `.claude/agents/scorer.md` (the single zero-tool scorer definition) instead, and neither session type gets both.
...
**The smoke launch.** Both risks below are settled by one non-matrix `claude` launch during the batch that builds `prepare-session`: a session against a throwaway scratch dir that confirms the generated agent definition loads and the declared `quarry-mcp` server starts, dispatching at most one trivial subagent that calls one quarry tool.
It writes nothing into any results tree, scores nothing, and is not a probe.
This is the one dispatch in scope for this task, and it is what makes "settled empirically" achievable rather than aspirational.
If the operator declines the smoke launch, the named default is: both risks move to the follow-up task, and `prepare-session` ships with the fallbacks below documented but unverified — which must then be stated in the README rather than left implicit.

**Known implementation risks, settled by that smoke launch, never assumed:**
...
`<results_root>/raw/<config-id>/<n>/` holds the per-run artifacts, gitignored: `transcript.jsonl`, `transcript.meta.json`, `answer.json`, `answer.redacted.json`, `usage.json`, `ingest.json`, `score.json`, `run.json`, and the run's launch inputs.
The launch inputs are **kept**, at parity with `write_run_inputs`: `ingest` copies the session's `settings.json` and its generated run agent definition into `<run_dir>/`, plus `.mcp.json` when the config has one.
Without the copy the record of what a run was launched with would die with the scratch dir, and under per-session generation there is no other place it survives.
`ingest.json` is written by `ingest` and marks a run as dispatched-and-gated; `run.json` is written last, by `record-score`, and remains the sole definition of a complete run (`is_complete`). Run sessions resume on the first marker, the scoring session on the gap between them, and `summarize` reads only the second.

### Subcommand surface

Eleven subcommands, split at each session boundary because only a live session can invoke the Agent Tool:

- `prepare-session` — for a run session (`--config-id <id> --rep <n>`): ensure task worktrees at their pins, build `quarry-mcp`, check the environment precondition, check the `~/.claude/skills/` blinding precondition for a `none` config, take the `.session-active` lock, write the scratch dir's `.mcp.json` (config permitting) / `settings.json` / run agent definition, install the skill to user scope, print the launch command. `--scoring` prepares the single scoring session instead (scorer definition and `Task`-only settings only). `--probe allowlist|denylist` prepares a probe session. `--release` clears the lock. `--run-model <id>` is a smoke-launch-only override.
- `next-run` — resume-aware; print this session's repetition if still pending (no `ingest.json`), its current **attempt index** (derived by counting `<n>.invalid-<k>` siblings), its full prompt, and its agent-definition name. Under `--scoring`, prints the next run directory with an `ingest.json` and no `run.json`.
- `warm` — the daemon warm-up call plus its `daemon.json` post-condition. Skipped for the cold config.
- `ingest` — enforce the single-flight predicate, locate the transcript by description, copy it and its meta in, copy the run's launch inputs in, extract `usage.json`, parse `answer.json`, run the gates, write `ingest.json`, print the outcome (`ingested` / `truncated` / `failed`). Never destroys evidence.
- `invalidate` — rename the failed run directory aside to `<n>.invalid-<k>`, taking `ingest.json` with it, and return the next attempt index; errors when `MAX_ATTEMPTS` is already exhausted, which is the matrix halt.
...
- Scratch-dir path derivation: `session_dir_template` substitutes `{config_id}` and `{n}`, giving a distinct path for each of the 45 run sessions, the scoring session, and the two probe sessions.
...
- `require_pins` enforcement split: the full check rejects each of `run_model`, `max_turns`, `run_effort`, `scorer.model`, `scorer.effort` when unset; `prepare-session`'s narrower check passes with `max_turns` unset and fails with `run_model` unset unless `--run-model` is supplied.
- The `.session-active` lockfile: `prepare-session` refuses when one exists naming another config, succeeds after `--release`, and `--release` on an absent file is not an error.
...
- Single-flight: ingesting rep `n` errors while rep `n-1` has none of `ingest.json`, `run.json`, or an exhausted attempt record, and succeeds once any is present.
- Resume filtering: a run session treats a repetition with an `ingest.json` as done; the scoring session treats a run directory with an `ingest.json` and no `run.json` as pending and one with both as done; `summarize` still counts only `run.json`.
...
- Transcript correlation: exactly one description match with its sibling `.jsonl` present succeeds; zero matches errors; two matches errors; exactly one match whose sibling `.jsonl` never appears within the bounded wait errors naming the meta path, and never falls back to another file.
- `task_text_for`'s section boundary, for both task files: extraction stops at the next `## ` heading, the `> ` / `>` blockquote prefixes are stripped, surrounding blank lines are trimmed, a missing heading errors, and an empty extracted body errors. The negative assertion matters most — task 04's extracted text must not contain the `burler.go` decoy or its scoring notes, and task 01's must not contain its fasit leads.
- `schema_for`: the impact schema comes from task 04's own `## Output schema (impact-analysis tasks)` section, the exploration schema from the benchmark README's `## Output schemas` section under its `**Exploration tasks:**` marker, selection is driven by the task's declared `schema` field, and an unknown schema or a missing section errors.
- Cold-cell disposition: all four outcomes (`confirmed-cold`, `partial`, `not-run`, `no-daemon-signal`) plus `not_run_causes` distinguishing `live_daemon_before_start` from `native_fallback_exhausted`, including the case where both causes occur and the reason text must name both.
- `probe-record`: writes `probe.json` with `allowlist_blocks` / `denylist_blocks`, halts on either being false, and captures `denial_shape_observed` verbatim from the deny-list probe's errored `tool_result`.
- Blinding-hygiene generators: a `none` config's generated run agent definition contains no case-insensitive "quarry"; the tracked `ladder-run` `SKILL.md`'s frontmatter contains no case-insensitive "quarry"; the skill precondition scan covers both the user-scope and plugin-cache roots, fails on a quarry-mentioning frontmatter under either for a `none` config, passes for a rung, treats an absent root as skipped rather than fatal, and reports every root scanned with its file count.
- Session-type write lists: a run session's scratch dir gets the run agent definition and never the scorer's; the scoring session's gets the scorer definition and never a run agent's or an `.mcp.json`.
- Provisional-marker propagation: a `usage.json` carrying `denied_tool_attempts_provisional` produces a `summary.json` cell whose `denied_tool_attempts` stats record carries the marker through, and clearing it in the runs clears it in the summary.
...
- **Q:** Session granularity, given that `settings.json` is per session? **A:** One session per config. **(Superseded — rounds 4 and 5 revised this to one session per *repetition*, 45 run sessions plus 1 scoring session = 46, once the session transcript was found to carry both prior answers and the unstripped fasit.)**
...
- **Q:** Where does a retry re-enter the loop — do `warm` and `restore-worktree` run per attempt or per repetition? **A:** [auto-pick] Per attempt, exactly where `run_matrix` puts them, with `restore-worktree` unconditional after every attempt. **Why:** a retry on an unrestored worktree misattributes one attempt's `observe_worktree_dirtied` to the next, and a retry that skipped the warm-up could dispatch against a self-expired daemon.
- **Q:** How does a cold repetition retry, given the session's `quarry-mcp` outlives the attempt? **A:** [auto-pick] It does not retry in-session — teardown, exit, and relaunch `prepare-session` for the next attempt. **Why:** re-clearing the state directory mid-session deletes the `daemon.json` whose pid `gate_cold_before` reads, so a surviving warm daemon would become invisible and attempt 2 would be reported cold against it.
- **Q:** The parent session transcript carries the scorer prompt, which embeds the unstripped fasit. **A:** A `none` config gets one session per repetition and its scoring is deferred to a separate scoring session. **(Superseded — round 5 extended the split to every config, since rung sessions had the identical fasit exposure and `gate_blinding` does not run for a rung: 45 run sessions plus 1 scoring session.)**
- **Q:** Are the run's `settings.json` / `mcp.json` still recorded per run? **A:** [auto-pick] Yes — `ingest` copies them, plus the run agent definition, into the run directory. **Why:** `write_run_inputs` wrote them per run today, and under per-session generation the scratch dir is the only other place they live.
- **Q:** What stops two sessions running concurrently? **A:** [auto-pick] A `<results_root>/.session-active` lockfile written by `prepare-session` and cleared by `prepare-session --release`, backed by a stated operator obligation. **Why:** `ingest`'s predicate cannot see another session; the lockfile catches the ordinary mistake without pretending to be a proof.
- **Q:** With `run_model` and `max_turns` both shipping `null`, can `prepare-session` run at all? **A:** [auto-pick] Yes — it enforces a narrower pin set that excludes `max_turns`, and takes a `--run-model` override for the smoke launch that is never written back to the file. **Why:** the ceiling is a gate-time value nothing about session preparation touches.
- **Q:** The fasit channel was closed for `none` sessions but rung sessions have the identical exposure, and `gate_blinding` never runs for a rung. **A:** [auto-pick] Make the model uniform — one session per repetition for every config, and all scoring in one dedicated scoring session. 46 sessions. **Why:** tool-provenance blinding and answer-key exposure are different problems; a rung's recall/precision is the primary measured outcome, so closing the channel for the blinded arms alone would have fixed the less important half.
- **Q:** With scoring deferred, `run.json` can no longer mark a run session's progress. **A:** [auto-pick] `ingest` writes an intermediate `ingest.json`; `run.json` and `is_complete` keep their existing meaning untouched. **Why:** run-session resume, scoring-session resume, and matrix completeness are three different questions, and collapsing them onto one marker is what broke the single-flight predicate.
- **Q:** Is `{n}` the session index or the repetition index? **A:** [auto-pick] The repetition index, uniformly, with `--rep` required for every run session. **Why:** with one session per repetition there is no longer any config for which "session index" and "repetition index" differ.
- **Q:** Is `/tmp/quarry-bench` dropped from `redact_text` too? **A:** [auto-pick] No — kept in the redactor, dropped only from the gate. **Why:** a gate that cannot fire reads as coverage that is not there; a redaction branch that never matches removes nothing and claims nothing. A redactor's job is to over-remove.
- **Q:** The skill-leak precondition scans `~/.claude/skills/`, which does not exist on this machine — installed skills live under the plugin cache. **A:** [auto-pick] Scan both roots, skip an absent root without erroring, and print every root scanned with its file count. **Why:** as written the check passed vacuously and closed nothing; printing the roots is what makes a vacuous pass visible instead of silent.
- **Q:** What if exactly one `.meta.json` matches but its sibling `.jsonl` is absent? **A:** [auto-pick] Bounded re-check for a not-yet-flushed transcript, then hard error naming the meta path — never a fallback to another file. **Why:** meta files with no sibling transcript exist on disk today, and the whole point of description-matching is that the copied transcript is provably the right one.
- **Q:** Is `.session-active` tracked? **A:** [auto-pick] Gitignored, via a new `results/**/.session-active` entry beside the existing `results/**/raw/` rule. **Why:** it is live session state, never part of a committed result.
...
- **Q:** How does a repo-tracked skill reach a session launched from `/tmp`? **A:** [auto-pick] `prepare-session` installs it to `~/.claude/skills/ladder-run/` (user scope) for every session type, and never copies it into a scratch dir. **Why:** the scratch dir is a different project scope than the repo, and it is also the blinded agent's own cwd — a copy there would be one `cat` from a fatal blinding finding. (Supersedes this entry's original scratch-dir answer, which round 3 showed contradicted the containment rule.)
- **Q:** `skill_listing` records enumerate every user-scope skill's name and description into the blinded agent's own transcript, with no tool call. **A:** [auto-pick] Constrain the `ladder-run` skill's frontmatter to be quarry-free with a unit test, and have `prepare-session` hard-fail a `none` config when any `~/.claude/skills/*/SKILL.md` frontmatter mentions quarry. **Why:** no cwd hygiene can close a channel the environment injects; the two mitigations close the channels the harness itself creates, and `gate_blinding` stays the detector over the whole transcript.
- **Q:** Does this task launch `claude` at all, given "no dispatch of any kind"? **A:** [auto-pick] Yes — exactly one smoke launch during the `prepare-session` batch, dispatching at most one trivial subagent, writing nothing to any results tree. **Why:** two implementation risks are stated as settled empirically, which is unachievable without a launch; the named default if the operator declines is that both risks move to the follow-up task and the fallbacks ship documented-but-unverified.
- **Q:** Is `max_turns: 60` still calibrated once the ceiling counts assistant records? **A:** [auto-pick] No — blank it to `null` like `run_model`, keep `require_pins` rejecting it, and redocument the field as "assistant records in a subagent transcript". **Why:** 60 was calibrated against the `claude -p` client's own turn accounting; carried over unchanged it is either dead or spuriously fatal with no way to tell which before a run.
...
- **Q:** How many sessions? **A:** 17 — 14 warm plus 3 cold. **(Superseded twice — round 4 raised it to 23, round 5 to 46: 45 one-repetition run sessions plus 1 zero-tool scoring session.)**
- **Q:** What is a session's step order? **A:** Fully serialized per repetition. **(Superseded — round 4 moved `restore-worktree` to immediately after `ingest` and inside the attempt loop, and round 5 removed scoring from run sessions entirely; see the per-repetition session loop decision for the current order.)**
...
- **Q:** `session_dir_template` keyed on `{config_id}` alone collides across the three cold sessions. **A:** Template becomes `/tmp/ladder-session-{config_id}-{n}`. **(Partly superseded — round 5 redefined `{n}` as the repetition index uniformly, required for every run session, since every config now has one session per repetition. The collision rationale stands.)**
