...
- Decision: **23** short Claude Code sessions total, launched by `ladderbench prepare-session --config-id <id>` (with `--rep <n>` where a session covers one repetition).
- Arithmetic, matching `ladder.yaml` and `plan_runs`: 15 configs = 14 non-cold + 1 cold, and of the 14 non-cold, 2 are `none` controls (`a0-none`, `b0-none`).
  - **12 rung sessions** — one per non-`none`, non-cold config, each running that config's 3 repetitions with scoring inline: 3 run agents + 3 scorer agents each, interleaved per the session loop below. 12 × 3 = 36 runs.
  - **6 `none` run sessions + 2 `none` scoring sessions** — a blinded config gets one session per repetition (1 run agent, no scoring), and one scoring session afterwards handling all 3 of its reps. See the blinding-channel decision below for why scoring is separated. 2 × 3 = 6 runs.
  - **3 cold sessions** — one per repetition of `a5-bundle-cold`: 1 run agent + 1 scorer agent each. 3 runs.
  - 36 + 6 + 3 = 45 runs, of which 42 are the main matrix and 3 the cold cell.
- Rationale: `settings.json` is one file per session, so a session hosting several configs can carry only one deny-list. The two-layer enforcement model below (agent-def allowlist as primary, session deny-list as backup) is only genuinely per-rung when the session is per-config. 23 manual launches is the honest cost of that model, once the blinded configs' per-repetition split is included. Supervision also stays granular, which is the point of the whole dispatch swap.
...
- **What a `none` session's scratch directory may contain — exhaustively.** The blinded run agent has `Read`/`Grep`/`Glob`/`Bash` and its cwd *is* the scratch dir, so anything written there is one `cat` away from the transcript, and `gate_blinding` makes any `mcp__quarry__` substring anywhere in the transcript fatal. A `none` session's scratch dir contains exactly three things and nothing else: the run agent definition (allowlist `Read`/`Grep`/`Glob`/`Bash`, naming nothing), the scorer agent definition (zero tools), and a `settings.json` whose `permissions.deny` is **`["Task"]` and nothing else**. No `.mcp.json`. Both agent definitions' names, descriptions, and bodies are constrained to be free of any case-insensitive "quarry" or `mcp__quarry__` substring, enforced by a unit test over the generator.
...
- **The parent session transcript is a third channel, and it carries the fasit.** The orchestrating session writes its own `~/.claude/projects/-tmp-ladder-session-<id>/<session-id>.jsonl`, which accumulates every `ladderbench` invocation and — critically — the scorer prompt `redact` prints, which embeds the task's **unstripped fasit**. A `none` config's session running reps 1–3 in one session would put rep 1's fasit within `Read` range of reps 2 and 3. That is not a marginal leak; it is the answer key.
- **Therefore a `none` config gets one session per repetition, and its scoring is deferred to a separate session.** A `none` config's session dispatches exactly one run agent and does no scoring at all. After its three run sessions complete, a single dedicated **scoring session** for that config runs `redact` → dispatch scorer → `record-score` for all three reps. No blinded agent ever shares a session with a fasit.
- Revised session count: 12 rung sessions (14 non-cold configs minus the 2 `none` controls, 3 reps each with inline scoring) + 2 `none` configs × (3 run sessions + 1 scoring session) = 8 + 3 cold sessions = **23 sessions**. The cost is real and accepted: the alternative is the fasit sitting in a blinded arm's own session transcript.
- Residual limit, stated plainly: even a per-repetition `none` session's transcript accumulates that session's own `ladderbench` command lines, which name the results root inside `repo_root`. A blinded agent that went looking for its own session transcript would find one — and `gate_blinding`'s repo-root check would fire fatally and invalidate the run, which is the correct outcome rather than a silent leak. Likewise the operator's user-scope skills are outside this harness's control between the precondition check and the launch, and `~/.claude/skills/` remains reachable by `Bash` from any cwd. These are hygiene measures that close the channels the harness itself creates, not a structural guarantee — the same class of limit `ladder/README.md` already acknowledges for the `none` arms generally. `gate_blinding` scans the whole transcript, `skill_listing` records included, and remains the detector.
...
- Decision: a session's loop is **fully serialized per repetition**, not batched into a run phase followed by a scoring phase, and `warm` / `restore-worktree` sit **inside the attempt loop**, exactly where `run_matrix` puts them. In full:
  ...
  for rep n in 1..reps:
    for attempt k in 1..MAX_ATTEMPTS:
      ...
      ingest                          # takes observe_worktree_dirtied before anything restores
...
- Decision: retained, as three dedicated short sessions run after all 20 non-cold sessions. Disposition logic (`confirmed-cold` / `partial` / `not-run` / `no-daemon-signal`, with `not_run_causes`) ports unchanged.
...
`<results_root>/raw/<config-id>/<n>/` holds the per-run artifacts, gitignored: `transcript.jsonl`, `transcript.meta.json`, `answer.json`, `answer.redacted.json`, `usage.json`, `score.json`, `run.json`, and the run's launch inputs.
...
- `prepare-session` — ensure task worktrees at their pins, build `quarry-mcp`, check the environment precondition, check the `~/.claude/skills/` blinding precondition for a `none` config, write the scratch dir's `.mcp.json` (config permitting) / `settings.json` / both agent definitions, install the skill to user scope, print the launch command.
...
- **Q:** Session granularity, given that `settings.json` is per session? **A:** One session per config — later revised to 23 total (12 rung, 6 `none` run + 2 `none` scoring, 3 cold). It is the only shape where both enforcement layers are genuinely per-rung, and 23 manual launches is the honest cost of the enforcement model plus the blinding split.
