...
// fencedJSONPattern is the one ```json ... ``` regex this suite compiles. Every call site across later
// batches (the schema extractor, the answer parser, the scorer-reply parser) selects a different match
// by position via ExtractFencedJSON's which argument, rather than re-deriving this pattern per call
