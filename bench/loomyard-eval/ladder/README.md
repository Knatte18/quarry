...
**Task-authoring rule for impact tasks:** every impact task must state explicitly, in the task text itself,
whether `_test.go` call sites belong in `callers_to_update`. Fasits are generated by running `go vet ./...`
against the pinned worktree after applying the signature change under test (not `go build` — `vet`
type-checks `_test.go` files too, and test call sites are part of the honest answer), so the fasit's
`callers_to_update` enumerates every call site `vet` flags, test files included, unless the task text says
otherwise. This exists because task 05 shipped without that statement: the task text asked for every call
site needing a new argument threaded in, the agent correctly included the `mergeresolve_test.go` sites, and
the fasit's scoring notes excluded them anyway — deflating precision to 0.133 in every cell for a reason
that has nothing to do with tool capability.
...
**Also set `LADDER_LOOMYARD_REPO`.** `source_repo` in every ladder file is the literal
`env:LADDER_LOOMYARD_REPO`, never a machine-specific checkout path -- `ladderbench` resolves it from this
environment variable at the point a session actually needs a real Loomyard checkout (building or removing
a disposable worktree), so the same committed ladder files run unchanged on any machine:

```
export LADDER_LOOMYARD_REPO=/path/to/your/loomyard/checkout
```

An unset or non-existent path fails loud with a clear error at that point, not silently. Every
`session_dir_template` is likewise repo-root-relative, not absolute -- `ladderbench` always runs from the
quarry repo root, so nothing else needs configuring for that.
