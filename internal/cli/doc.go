...
// "expand" takes a glyph only.
//
// A target is handed to the facade verbatim, with no path arithmetic and no stat ever applied to
// it, whenever the verb takes a glyph. This is because a glyph's unit is repository-relative by
// the grammar's own definition — cwd arithmetic on it would corrupt it, the same way rebasing a
// remote URL against a local directory would. "toc" is the only verb that still takes a path, and
// it is the only verb this package still converts with internal/repopath before the engine sees
// the target.
//
...
// exactly as it would any other malformed input. "toc" takes a path only, and a "#" in any segment
// of that path is an explicit error there too — internal/repopath.RepoRelTarget returns
// quarry.ErrTargetHasSeparator rather than reclassifying the target as a glyph. The grammar's
// separator rule holds everywhere a target is taken, not only at the two verbs that read one as a
// glyph.
package cli
