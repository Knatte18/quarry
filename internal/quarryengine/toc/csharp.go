...
// Header implements Strategy by walking LeadingBlocks in order and, for each block, stripping it
// with StripComment(b.Raw, "///") followed by StripComment(result, "//") so both the XML-doc and the
// plain comment forms are handled. IsDirectiveBlock("csharp", ...) is evaluated against that
// comment-stripped text, deliberately before StripXMLDocTags runs: csharp's directive rule matches
// the literal "<auto-generated" tag text, so checking after tag removal would make the check
// permanently unreachable — every auto-generated banner would silently pass as an ordinary header.
// The first block for which the check is false is returned with StripXMLDocTags then applied, so an
// auto-generated block is skipped as a header and the next block, if any, is taken instead.
//
...
func (csharpStrategy) Header(root *ts.Node, src []byte) string {
	for _, block := range LeadingBlocks(root, src) {
		commentStripped := StripComment(StripComment(block.Raw, "///"), "//")
		if IsDirectiveBlock("csharp", block.StartLine, commentStripped) {
			continue
		}
		return StripXMLDocTags(commentStripped)
	}
	return ""
}
