...
- **Annex ladder (`ladder-annex.yaml`, `run-annex.sh`) — built, tested, dry-run, not to be run now.**
  ...
  **2026-09-02, operator: the same question may be testable in Millhouse instead, which is already in
  production and already has the shape.** A mill implementer dispatch is exactly the annex
  precondition — the plan is known before the agent starts, the dispatch is non-interactive, and the
  caller knows which symbols the batch touches. A pre-computed symbol/index pack attached to the
  implementer brief is `b11-annex-plan` in the ladder, but measured on the real workload rather than on
  task 04. If that route is taken, the ladder's value drops to the two cells the live system cannot
  isolate: `a8-annex-toc-wide` (over-injection cost) and `b12-annex-plan-degraded` (does the agent
  verify the attachment or trust it — recall 1.0 vs 0.67). Not started; deliberately not today. The
